...
export const QUALITIES: Record<string, { pcs: number[]; about: string }> = {
  ...
  min7: { pcs: [0, 3, 7, 10], about: 'minor seventh — dark, ordinary jazz minor' },
  dom7: { pcs: [0, 4, 7, 10], about: 'dominant seventh — wants to move' },
  alt: { pcs: [0, 4, 6, 10], about: 'altered dominant — tense, b5, no perfect fifth' },
  halfdim: { pcs: [0, 3, 6, 10], about: 'half diminished — minor seventh with a flattened fifth' },
  minmaj: { pcs: [0, 3, 7, 11], about: 'minor major seventh — tonic minor, a little haunted' },
  sus: { pcs: [0, 5, 7, 10], about: 'suspended dominant — no third, hanging' },
  lydian: { pcs: [0, 4, 6, 11], about: 'lydian major — raised fourth, floating' },
}
...
export function voice(root: number, quality: string, previous: number[], low = 48, high = 72): number[] {
