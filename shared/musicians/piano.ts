...
export function pianoQuestions(heard: Heard): Questions {
  return forMusician('piano', {
    ...
    quality: {
      type: 'choice',
      instructions:
        'What quality of chord, given what you just heard? Free jazz: you may clash. You may also agree.',
      criteria: qualityCriteria(),
    },
    voicing: {
      type: 'choice',
      instructions:
        'How do you put it under your hands? Rootless is the late-1950s default when a bass is walking. Not a rule.',
      criteria: VOICINGS,
    },
    time: {
      type: 'choice',
...
export function assemblePiano(
  answers: Answers,
  heard: Heard,
  seed: number,
): { part: PianoPart; decisions: Decision[] } {
  const rest = sampledNoul(answers, 'rest', seed)
  const root = sampledChoice(answers, 'root', 'C', seed, 0, 1.2)
  const quality = sampledChoice(answers, 'quality', 'dom7', seed, 0, 1.15)
  const voicing = sampledChoice(answers, 'voicing', 'rootless', seed, 0, 1.15)
  const time = sampledChoice(answers, 'time', 'on', seed, 0, 1.15)
  ...
  if (root.decision)
    decisions.push({ ...root.decision, seat: 'piano', label: 'Root' })
  if (quality.decision)
    decisions.push({ ...quality.decision, seat: 'piano', label: 'Quality' })
  if (voicing.decision)
    decisions.push({ ...voicing.decision, seat: 'piano', label: 'Voicing' })
  if (time.decision)
    decisions.push({ ...time.decision, seat: 'piano', label: 'Time' })
  ...
  const part: PianoPart = {
    ...
    time: time.key,
    heard: rest
      ? `laid out after ${heard.barsSoFar}`
      : `${root.key}${quality.key === 'maj7' ? 'Δ' : quality.key === 'min7' ? '-' : quality.key} ${voicing.key} ${time.key}`,
  }
